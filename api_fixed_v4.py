...
CORS(app, supports_credentials=True, origins=[
    'https://checking.shorthandonlineexam.in',
    'https://www.shorthandexam.in',
    'https://dev.shorthandexam.in',
    'http://localhost:3000',
    'http://localhost:3001',
...
def preprocess_text(text):
    ...
    text = re.sub(r'/{1,4}', ' ', text)  # strip /  //  ///  //// markers before tokenizing
...
def compare_texts(text1, text2, ignore_list):
    ...
    LINE_MARKERS = ['////', '///', '//', '/']
    ignore_list = list(set([preprocess_text(word).lower() for word in ignore_list] + LINE_MARKERS))
    ...
    all_spelling_parts = spelling_left_parts | spelling_right_parts
    if all_spelling_parts:
        missed = [w for w in missed
                  if not (isinstance(w, str) and w.lower() in all_spelling_parts)]
        added = [w for w in added
                 if not (isinstance(w, str) and w.lower() in all_spelling_parts)]
        # Fix colored_words: turn leaked red/green parts back to black
        for i, cw in enumerate(colored_words):
            if cw['color'] in ('red', 'green') and isinstance(cw['word'], str) and cw['word'].lower() in all_spelling_parts:
                colored_words[i] = {'word': cw['word'], 'color': 'black'}
