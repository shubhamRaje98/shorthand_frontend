...
const FinalPassageTextlog = () => {
    ...
    const handleSubmit = async () => {
      try {
          const response = await axios.post(
              `https://shorthandonlineexam.in/submit-passage-review/${subjectId}/${qset}`, 
              {}, 
    ...
    useEffect(() => {
      const fetchPassages = async () => {
          try {
              const response = await axios.get(`https://shorthandonlineexam.in/expert-assigned-passages/${subjectId}/${qset}`, { withCredentials: true });
              if (response.status === 200) {
    ...
    useEffect(() => {
      const sendActivePassageData = async () => {
          try {
              ...
              const response = await axios.post('https://shorthandonlineexam.in/active-passage', {
                  subjectId,
                  qset,
                  activePassage,
              }, { withCredentials: true });
    ...
    const handleAddIgnoreWord = useCallback(async (word) => {
      try {
        const response = await axios.post('https://shorthandonlineexam.in/add-ignore-word', {
          subjectId,
          qset,
          activePassage,
          newWord: word
    ...
    const handleUndoWord = useCallback(async (wordToRemove) => {
      try {
        const response = await axios.post('https://shorthandonlineexam.in/undo-word', {
          subjectId,
          qset,
          activePassage,
          wordToRemove
    ...
    const handleClearIgnoreList = useCallback(async () => {
      try {
        const response = await axios.post('https://shorthandonlineexam.in/clear-ignore-list', {
          subjectId,
          qset,
          activePassage
        }, { withCredentials: true });
