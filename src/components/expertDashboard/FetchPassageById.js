...
const FetchPassageById = () => {
    ...
    useEffect(() => {
      const fetchPassages = async () => {
          try {
                const response = await axios.get(`http://localhost:3001/student-passages/${subjectId}/${qset}/${studentId}`, { withCredentials: true });
    ...
    useEffect(() => {
      const fetchAudio = async () => {
        try {
          const response = await axios.get(`http://localhost:3001/get-student-audio-id/${subjectId}/${qset}/${studentId}`, { withCredentials: true });
    ...
    useEffect(() => {
      const sendActivePassageData = async () => {
          try {
              ...
              const response = await axios.post('http://localhost:3001/student-active-passage', {
                  subjectId,
                  qset,
                  activePassage,
                  studentId
    ...
    const handleAddIgnoreWord = useCallback(async (word) => {
      try {
        const response = await axios.post('http://localhost:3001/student-add-ignore-word', {
          subjectId,
          qset,
          activePassage,
          newWord: word,
    ...
    const handleUndoWord = useCallback(async (wordToRemove) => {
      try {
        const response = await axios.post('http://localhost:3001/student-undo-word', {
          subjectId,
          qset,
          activePassage,
          wordToRemove, 
    ...
    const handleClearIgnoreList = useCallback(async () => {
      try {
        const response = await axios.post('http://localhost:3001/student-clear-ignore-list', {
          subjectId,
          qset,
          activePassage,
          studentId
