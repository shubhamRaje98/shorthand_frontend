...
const ExpertDashboard = () => {
    ...
    useEffect(() => {
        const fetchExpertDetails = async () => {
            try {
                const response = await axios.get('http://52.66.236.172:3000/expert-details', { withCredentials: true });
    ...
    useEffect(() => {
        const heartbeatInterval = setInterval(() => {
            axios.post('http://52.66.236.172:3000/expert-heartbeat', {}, { withCredentials: true })
                .catch((error) => {
    ...
    const handleLogout = async () => {
        try {
            await axios.post('http://52.66.236.172:3000/expert-logout', {}, { withCredentials: true });
            navigate('/expert-login', {replace: true});
        } catch (error) {
