...
const QSet = () => {
  ...
  useEffect(() => {
    const fetchQSets = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`https://shorthandonlineexam.in/qsets/${subjectId}`, { withCredentials: true });
  ...
  const handleQSetClick = async (qsetObj) => {
    try {
      const response = await axios.post(`https://shorthandonlineexam.in/assignStudent/${subjectId}/${qsetObj.qset}`, {}, { withCredentials: true });
      if (response.status === 200) {
