...
const QSet = () => {
  ...
  useEffect(() => {
    const fetchQSets = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`http://43.205.192.129:3000/qsets/${subjectId}${isHeld ? '?held=true' : ''}`, { withCredentials: true });
  ...
  const handleQSetClick = async (qsetObj) => {
    try {
      ...
      const response = await axios.post(
        `http://43.205.192.129:3000/assignStudent/${subjectId}/${qsetObj.qset}?held=${isHeld}`, 
        {}, 
