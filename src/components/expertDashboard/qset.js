...
const QSet = () => {
  ...
  useEffect(() => {
    const fetchQSets = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`http://localhost:3001/qsets/${subjectId}${isHeld ? '?held=true' : ''}`, { withCredentials: true });
  ...
  const handleQSetClick = async (qsetObj) => {
    try {
      ...
      const response = await axios.post(
        `http://localhost:3001/assignStudent/${subjectId}/${qsetObj.qset}?held=${isHeld}`, 
        {}, 
