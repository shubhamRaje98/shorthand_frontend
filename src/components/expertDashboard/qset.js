...
const QSet = () => {
  ...
  useEffect(() => {
    const fetchQSets = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`http://3.111.171.201:3000/qsets/${subjectId}${isHeld ? '?held=true' : ''}`, { withCredentials: true });
  ...
  const handleQSetClick = async (qsetObj) => {
    try {
      ...
      const response = await axios.post(
        `http://3.111.171.201:3000/assignStudent/${subjectId}/${qsetObj.qset}?held=${isHeld}`, 
        {}, 
