...
const QSet = () => {
  ...
  useEffect(() => {
    const fetchQSets = async () => {
      try {
        ...
        let url = `http://checking.shorthandonlineexam.in/qsets/${subjectId}${isHeld ? '?held=true' : ''}`;
  ...
  const handleQSetClick = async (qsetObj) => {
    try {
      ...
      let url = `http://checking.shorthandonlineexam.in/assignStudent/${subjectId}/${qsetObj.qset}/${departmentId}/${examType}`;
