...
const QSet = () => {
  ...
  useEffect(() => {
    const fetchQSets = async () => {
      try {
        ...
        let url = `https://www.shorthandonlineexam.in/qsets/${subjectId}${isHeld ? '?held=true' : ''}`;
  ...
  const handleQSetClick = async (qsetObj) => {
    try {
      ...
      let url = `https://www.shorthandonlineexam.in/assignStudent/${subjectId}/${qsetObj.qset}/${departmentId}/${examType}`;
