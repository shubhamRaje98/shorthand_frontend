...
const QSet = () => {
  ...
  useEffect(() => {
    const fetchQSets = async () => {
      try {
        ...
        let url = `http://45.119.47.81:3000/qsets/${subjectId}${isHeld ? '?held=true' : ''}`;
  ...
  const handleQSetClick = async (qsetObj) => {
    try {
      ...
      let url = `http://45.119.47.81:3000/assignStudent/${subjectId}/${qsetObj.qset}/${departmentId}/${examType}`;
