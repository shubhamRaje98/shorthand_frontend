...
const QSet = () => {
  ...
  useEffect(() => {
    const fetchQSets = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`http://45.119.47.81:3000/qsets/${subjectId}${isHeld ? '?held=true' : ''}`, { withCredentials: true });
  ...
  const handleQSetClick = async (qsetObj) => {
    try {
      ...
      const response = await axios.post(
        `http://45.119.47.81:3000/assignStudent/${subjectId}/${qsetObj.qset}?held=${isHeld}`, 
        {}, 
