...
const DepartmentDashboard = () => {
  ...
  // Fetch initial filter options (batches, centers, dates) separately for better performance
  const fetchFilterOptions = async () => {
    try {
      const response = await axios.get(
        "http://localhost:3000/department-filter-options",
  ...
  useEffect(() => {
    ...
    debouncedFetchData();
  }, [batchNo, subject, loginStatus, batchDate, center, exam_type]);
  ...
  const indexOfLastItem =
    itemsPerPage === "all" ? data.length : currentPage * Number(itemsPerPage);
  const indexOfFirstItem =
    itemsPerPage === "all" ? 0 : indexOfLastItem - Number(itemsPerPage);
  const currentItems =
    itemsPerPage === "all"
      ? data
  ...
  for (
    ...
    Math.ceil(
      data.length /
        (itemsPerPage === "all" ? data.length : Number(itemsPerPage))
    );
    i++
  ) {
    pageNumbers.push(i);
      ...
      <div className="home-container">
        <div className="dept-container-fluid">
          ...
          <div className="dept-row mb-3">
            ...
            <div className="dept-col-md-3 dept-col-sm-6 mb-2">
              <label htmlFor="itemsPerPage" className="dept-form-label">
                Rows per page:
              </label>
              <select
                className="dept-form-select"
            ...
            <div className="dept-col-md-3 dept-col-sm-6 mb-2">
              <button
                ...
                className="dept-btn dept-btn-primary dept-export-btn"
              >
                Export to Excel
              </button>
            </div>
