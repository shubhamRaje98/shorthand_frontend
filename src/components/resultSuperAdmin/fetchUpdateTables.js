...
const FetchUpdateTable = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await axios.post('http://43.205.192.129:3000/fetch-update-resultdb', fetchParams);
  ...
  const handleSave = async (index) => {
    try {
      await axios.put(`http://43.205.192.129:3000/update-table/${selectedTable}/${editingData.student_id}`, editingData);
  ...
  const handleAddNewEntry = async () => {
    try {
      const response = await axios.post(`http://43.205.192.129:3000/add-entry/${selectedTable}`, newEntryData);
