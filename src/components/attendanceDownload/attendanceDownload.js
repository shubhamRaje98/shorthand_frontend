...
const AttendanceDownload = () => {
    ...
    const fetchController = async () => {
        try {
            const response = await axios.post('http://localhost:3002/get-batch-controller-password', {
                batchNo,
                departmentId
            });
    ...
    const handleDownload = async (reportType) => {
        ...
        try {
            const response = await axios({
                url: `http://localhost:3002/center/${reportType}-pdf-download`,
                method: 'POST',
    ...
    const handleExcelDownload = async () => {
        ...
        try {
            const response = await axios({
                url: 'http://localhost:3002/center/studentId-password',
                method: 'POST',
                data: { batchNo, departmentId },
                responseType: 'blob',
                headers: {
