...
const ResetRequestsAdmin = () => {
  ...
  const fetchRequests = async () => {
    try {
      const response = await axios.get('http://3.111.171.201:3000/admin/center-request-datas');
  ...
  const handleApproveReject = async (requestId, action) => {
    try {
      await axios.post('http://3.111.171.201:3000/admin/approve-reset-request', {
        requestId,
        action
      });
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://3.111.171.201:3000/centerrequest/${id}`);
