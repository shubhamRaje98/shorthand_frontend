...
const SubmitAndDone = () => {
  ...
  useEffect(() => {
    const fetchInitialOptions = async () => {
      try {
        setLoading(true);
        const res = await axios.get('https://www.shorthandonlineexam.in/review-logs/filter-options', {
          params: { table: 'expertreviewlog' }
        });
        setOptions({
          subjects: res.data.data?.subjects || [],
  ...
  const fetchLogsWithFilters = async (customFilters) => {
    ...
    try {
      ...
      const res = await axios.get('https://www.shorthandonlineexam.in/expert-review-logs', { params });
      setLogs(res.data.data || []);
  ...
  const resetLogs = async () => {
    ...
    try {
      ...
      const res = await axios.post('https://www.shorthandonlineexam.in/expert-review-logs/reset', data);
      setMessage(res.data.message || 'Reset successful');
      fetchLogsWithFilters(filters);
    } catch (err) {
      console.error('Failed to reset logs:', err);
