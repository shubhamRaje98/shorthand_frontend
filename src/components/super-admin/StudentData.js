...
const StudentData = () => {
    ...
    useEffect(() => {
        const fetchCenters = async () => {
            try {
                const response = await axios.get('http://localhost:3002/reset-centers');
    ...
    useEffect(() => {
        const fetchResetRequests = async () => {
            try {
                const response = await axios.get(`http://localhost:3002/get-pending-requests?center=${selectedCenter}`);
    ...
    const handleSubmit = async (id) => {
        ...
        setError('');
        try {
            const response = await axios.post('http://localhost:3002/admin/student-data', { student_id: id });
            console.log(response.data);
    ...
    const handleDelete = async (student_id , reset_id) => {
        ...
        setError('');
        try {
            const response = await axios.post('http://localhost:3002/reject-reset-request', { student_id: student_id.toString() ,reset_id:reset_id.toString() });
