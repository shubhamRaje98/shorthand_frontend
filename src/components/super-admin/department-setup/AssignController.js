...
const AssignController = () => {
    ...
    try {
      ...
      if (persisted) {
        ...
        if (data.controllerData) {
          setFormData(data.controllerData);
          // If departmentId exists in persisted data, fetch its batches
          if (data.controllerData.departmentId) {
            fetchBatchesByDepartment(data.controllerData.departmentId);
          }
        }
      }
    } catch (error) {
  ...
  const clearStepData = () => {
    try {
      const existingData = JSON.parse(localStorage.getItem('departmentSetupData') || '{}');
      const { controllerData, ...rest } = existingData;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage(`Controller assigned successfully! Password: ${response.data.data.controller_pass}`);
      clearStepData(); // Clear this step's data on success
