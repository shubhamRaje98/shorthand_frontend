...
const AdminBlankSubmissionsView = () => {
    ...
    const fetchBlankSubmissions = async () => {
        ...
        try {
            ...
            const url = `http://checking.shorthandonlineexam.in/api/blank-submissions/admin/all${params.toString() ? '?' + params.toString() : ''}`;
    ...
    const handleMarkAsViewed = async (id) => {
        try {
            const response = await axios.post(
                `http://checking.shorthandonlineexam.in/api/blank-submissions/admin/mark-viewed/${id}`,
                {},
