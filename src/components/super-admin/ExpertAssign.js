...
const ExpertAssign = () => {
  ...
  const fetchData = async () => {
    try {
      let url = `http://checking.shorthandonlineexam.in/get-student-count-expert?${stage === 'stage1' ? 'stage_1' : 'stage_3'}=true`;
  ...
  const fetchExperts = async () => {
    try {
      const response = await axios.get('http://checking.shorthandonlineexam.in/get-experts');
  ...
  const fetchSummaryData = async () => {
    try {
      const response = await axios.get(`http://checking.shorthandonlineexam.in/get-student-summary-expert?${stage === 'stage1' ? 'stage_1' : 'stage_3'}=true`);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post('http://checking.shorthandonlineexam.in/assign-expert', {
  ...
  const handleUnassign = async () => {
    ...
    try {
      const response = await axios.post('http://checking.shorthandonlineexam.in/unassign-expert', {
        department: selectedDepartment,
        subject: selectedSubject,
        qset: selectedQset.qset,
        expertId: selectedExpert.expertId,
