...
//       const response = await axios.post('http://localhost:3004/upload-student-data', formData, {
//         headers: {
...
//       const url = `http://localhost:3004/download-student-hall-ticket/${student.seatNo}`;
...
//       window.open('http://localhost:3004/download-all-hall-tickets', '_blank');
...
function HallticketsGeneration() {
  ...
  const uploadExcelFile = async () => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:3004/upload-halltickets', formData, {
        headers: {
          'Content-Type': 'multipart/form-data'
        }
      });
    ...
    try {
      ...
      reader.onload = async (e) => {
        try {
          ...
          // Map Excel data to match backend fields
          const mappedStudents = excelData.map(row => ({
            seatNo: row['student_id']?.toString() || '',
            instituteId: row['instituteId']?.toString() || '',
            candidateName: row['fullname'] || '',
  ...
  const calculateReportingTime = (startTime) => {
  ...
  const downloadSingleHallTicket = async (student) => {
    try {
      ...
      const url = `http://localhost:3004/download-hall-ticket/${student.seatNo}`;
      window.open(url, '_blank');
      
      setSuccess(`Hall ticket download initiated for student ID: ${student.seatNo}`);
    } catch (error) {
      console.error('Error downloading hall ticket:', error);
      setError(`Failed to download hall ticket: ${error.message}`);
    } finally {
        ...
        <Modal.Body>
          <Row>
            <Col md={8}>
              ...
              <p><strong>Center Address:</strong> {previewStudent.centerAddress}</p>
              <p><strong>Password:</strong> {previewStudent.password}</p>
              <p><strong>Disability:</strong> {previewStudent.handicap}</p>
            </Col>
            <Col md={4} className="text-center">
              {previewStudent.image ? (
                <div>
            ...
            <Row className="mb-4">
              <Col md={6}>
                <ListGroup variant="flush" className="small">
                  ...
                  <ListGroup.Item><strong>instituteId</strong>: Institute code</ListGroup.Item>
                  <ListGroup.Item><strong>fullname</strong>: Student's full name</ListGroup.Item>
                  <ListGroup.Item><strong>MOTHER NAME</strong>: Mother's name</ListGroup.Item>
                  <ListGroup.Item><strong>center</strong>: Center number</ListGroup.Item>
                  <ListGroup.Item><strong>subjectsId</strong>: Subject code</ListGroup.Item>
                  <ListGroup.Item><strong>batchNo</strong>: Batch number</ListGroup.Item>
                </ListGroup>
              ...
              <Col md={6}>
                <ListGroup variant="flush" className="small">
                  <ListGroup.Item><strong>batchdate</strong>: Exam date (YYYY-MM-DD)</ListGroup.Item>
                  <ListGroup.Item><strong>start_time</strong>: Exam start time (HH:MM)</ListGroup.Item>
                  <ListGroup.Item><strong>password</strong>: Student password</ListGroup.Item>
                  ...
                  <ListGroup.Item><strong>center_address</strong>: Exam center address</ListGroup.Item>
