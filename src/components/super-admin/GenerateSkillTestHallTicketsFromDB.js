...
function GenerateSkillTestHallTicketsFromDB() {
  ...
  // Load centers from database (filtered by departmentId)
  ...
  // Load batches from database (filtered by departmentId)
  ...
  // Load students from database with filters (departmentId is mandatory)
  const loadStudents = async () => {
    try {
      ...
      // Build URL with departmentId as mandatory parameter
  ...
  const downloadSingleHallTicket = async (student) => {
    try {
      ...
      const url = `http://localhost:3000/api/skilltest-halltickets/db/download-hall-ticket/${student.student_id}?departmentId=${departmentId || ''}`;
      console.log('Generated DB download URL:', url);

      // Open the download URL in a new tab
      window.open(url, '_blank');
      
      setSuccess(`Hall ticket download initiated for student ID: ${student.student_id}`);
    } catch (error) {
  ...
  // Download all hall tickets from database
  const downloadAllHallTickets = async () => {
    try {
      ...
      // Build URL with departmentId as mandatory parameter
      let url = `http://localhost:3000/api/skilltest-halltickets/db/download-all-hall-tickets?departmentId=${departmentId}`;
      ...
      <Row className="align-items-center mb-4">
        ...
        <Col xs="auto">
          <div className="d-flex align-items-center">
            ...
            {departmentId ? (
              <Badge bg="secondary" className="fs-6 me-3">
                Department ID: {departmentId}
              </Badge>
            ) : (
              <Badge bg="danger" className="fs-6 me-3">
          ...
          <Card>
            ...
            <Card.Body>
              ...
              <div className="text-muted">
                <small>
                  All PDFs will be compressed into a single ZIP file.
                </small>
              </div>
            </Card.Body>
          </Card>
