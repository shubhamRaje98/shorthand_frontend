...
const FetchUpdateTable = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await axios.post('http://13.127.206.181:3000/fetch-update-tables', fetchParams);
  ...
  const handleSave = async (index) => {
    try {
      await axios.put(`http://13.127.206.181:3000/update-table/${selectedTable}/${editingData.student_id}`, editingData);
  ...
  const handleAddNewEntry = async () => {
    try {
      const response = await axios.post(`http://13.127.206.181:3000/add-entry/${selectedTable}`, newEntryData);
