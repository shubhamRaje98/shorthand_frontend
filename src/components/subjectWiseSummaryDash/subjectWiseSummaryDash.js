...
const SubjectWiseResultSummary = () => {
  ...
  const fetchSummaryData = async () => {
    try {
      ...
      const eventSource = new EventSource('http://52.66.236.172:3000/recalculate-progress');
      eventSource.onmessage = (event) => {
        const data = JSON.parse(event.data);
        setProgress(data.progress);
      };

      const response = await axios.get('http://52.66.236.172:3000/recalculate-result');
