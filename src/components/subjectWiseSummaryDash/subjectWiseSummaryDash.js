...
const SubjectWiseResultSummary = () => {
  ...
  const fetchSummaryData = async () => {
    try {
      ...
      const eventSource = new EventSource('https://checking.shorthandonlineexam.in/recalculate-progress');
      eventSource.onmessage = (event) => {
        const data = JSON.parse(event.data);
        setProgress(data.progress);
      };

      const response = await axios.get('https://checking.shorthandonlineexam.in/recalculate-result');
