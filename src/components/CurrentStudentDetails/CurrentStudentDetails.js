...
const CurrentStudentDetails = () => {
    ...
    const fetchDepartments = async () => {
        try {
            const response = await axios.get('http://localhost:3002/departments', { withCredentials: true });
    ...
    const fetchData = async () => {
        ...
        setError('');
        try {
            let url = 'http://localhost:3002/center-batches';
            if (departmentId) {
    ...
    const fetchAllData = async () => {
        ...
        setError('');
        try {
            let url = `http://localhost:3002/get-current-student-details`;
